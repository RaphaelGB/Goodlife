...
namespace Goodlife
{
    public partial class Visualizacao_func : Form
    {
        //Variáveis
        //Banco de dados
        ...
        {
            tTp_dicalist.Show("Pressione 'Enter' para abrir o Prontuário", lst_agenda);
        }
        */
        ...
        public Visualizacao_func(String nome)
        {
            InitializeComponent();
            carregar_contatos();
            nome_func = nome;
        ...
        private void lst_agenda_KeyDown(object sender, KeyEventArgs e)
        {
            string tecla = e.KeyCode.ToString();
            if (tecla == "Return")
            {
                this.Hide();
                Prontuario FrmPront= new Prontuario(nome_func, especializacao, nome_cliente, dt_consulta, horario, retorno, id_agend);
                FrmPront.FormClosed += (s, args) => this.Close();
                FrmPront.ShowDialog();
            }
        }
    }
